<x-app-layout>
    <div class="p-4 sm:ml-64">
        <div class="container mt-5">
            <h1 class="mb-4 ml-3 text-3xl">Manage Users</h1>
                            ...
                            <form id="editForm" class="p-4 md:p-5">
                                <div class="grid gap-4 mb-4 grid-cols-2">
                                    <div class="col-span-2">
                                        <label for="name"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Name</label>
                                        <input type="text" name="name_edit" id="name_edit" value="{{ old('name_edit') }}"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                            placeholder="Type user name">
                                    ...
                                    <div class="col-span-2">
                                        <label for="password"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Password</label>
                                        <input type="password" name="password_edit" id="password_edit" value="{{ old('password_edit') }}"
                                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    ...
                                    <div class="col-span-2">
                                        <label for="role"
                                            class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Role</label>
                                            <select id="role_edit" name="role_edit" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                                <option selected value="">Choose role</option>
                                                <option value="Admin">Admin</option>
                                                <option value="Cashier">Cashier</option>
                                            </select>
    ...
    @push('scripts')
        ...
        <script>
            ...
            $(document).ready(function() {
                ...
                //update supplier
                $('#empTable').on('click', '.updateUsers', function() {
                    var id = $(this).data('id');
                    $.ajax({
                        url: `/users/${id}/edit`,
                        method: 'GET',
                        success: function(response) {
                            ...
                            $('#editForm').find('input[name="name_edit"]').val(response.data[0].name);
                            $('#editForm').find('input[name="user_id"]').val(response.data[0].id);
                            $('#editForm').find('input[name="email_edit"]').val(response.data[0]
                                .email);
                            $('#editForm').find('input[name="password_edit"]').val(response.data[0].password);
                            $('#role_edit option[value="'+response.data[0].role+'"]').attr("selected", "selected");
                            $('#updateModal').show();
                        }
                    });
                });
                ...
                $('#editForm').submit(function(e) {
                        e.preventDefault();
                        let id = $(this).find('input[name="user_id"]').val();
